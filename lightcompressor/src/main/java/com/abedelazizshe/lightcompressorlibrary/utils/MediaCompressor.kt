...
import com.abedelazizshe.lightcompressorlibrary.MediaCompressionProgressListener
import com.abedelazizshe.lightcompressorlibrary.MediaQuality
...
object MediaCompressor {
    ...
    fun compressVideo(
        ...
        listener: MediaCompressionProgressListener,
    ): AnotherResult {
        ...
        if (isMinBitRateEnabled && bitrate <= MIN_BITRATE)
            return AnotherResult(success = false, failureMessage = INVALID_BITRATE)
        ...
        if (!file.canRead()) return AnotherResult(
            success = false,
        ...
        if (newWidth != 0 && newHeight != 0) {
            ...
            return AnotherResult(success = true, failureMessage = null)
        }

        return AnotherResult(success = false, failureMessage = "Something went wrong, please try again")
    ...
    private fun getBitrate(
        ...
        return when (quality) {
            MediaQuality.VERY_LOW -> (bitrate * 0.08).roundToInt()
            MediaQuality.LOW -> (bitrate * 0.1).roundToInt()
            MediaQuality.MEDIUM -> (bitrate * 0.2).roundToInt()
            MediaQuality.HIGH -> (bitrate * 0.3).roundToInt()
            MediaQuality.VERY_HIGH -> (bitrate * 0.5).roundToInt()
        }
    }
