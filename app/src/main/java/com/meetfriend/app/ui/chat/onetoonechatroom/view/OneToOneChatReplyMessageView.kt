...
import com.meetfriend.app.databinding.ViewReplyMessageBinding
...
class OneToOneChatReplyMessageView(context: Context, canSendMessage: Int) :
    ...
    private lateinit var binding: ViewReplyMessageBinding
    ...
    private fun inflateUi() {
        val view = View.inflate(context, R.layout.view_reply_message, this)
        layoutParams = LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT)
        binding = ViewReplyMessageBinding.bind(view)

        MeetFriendApplication.component.inject(this)

        binding.apply {
            ivReplyUserProfileImage.throttleClicks().subscribeAndObserveOnMainThread {
                messageItemClicksSubject.onNext(MessageAction.ViewProfile(messageInfo))
            }.autoDispose()

            tvReplyUserName.throttleClicks().subscribeAndObserveOnMainThread {
                messageItemClicksSubject.onNext(MessageAction.ViewProfile(messageInfo))
            }.autoDispose()
    ...
    fun bind(messageInfo: MessageInfo, isLastMessageFromSender: Boolean) {
        ...
        binding.ivReplyUserProfileImage.visibility = if (isLastMessageFromSender) View.VISIBLE else View.INVISIBLE

        if (messageInfo.replyId != 0) {
            binding.tvOriginalMessageUserName.text = messageInfo.replyName
            binding.tvOriginalMessage.text = messageInfo.replyMessage
            binding.rlReplyMessageContainer.visibility = View.VISIBLE

            Glide.with(context)
                .load(messageInfo.senderProfile)
                .placeholder(R.drawable.ic_empty_profile_placeholder)
                .error(R.drawable.ic_empty_profile_placeholder)
                .into(binding.ivReplyUserProfileImage)

            binding.tvReplyUserName.text = messageInfo.senderName
            binding.tvReplyMessage.text = messageInfo.message
        }
    }
}
