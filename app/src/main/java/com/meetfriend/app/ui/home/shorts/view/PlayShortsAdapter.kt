package com.meetfriend.app.ui.home.shorts.view

import android.content.Context
import android.view.View
import android.view.ViewGroup
import androidx.fragment.app.FragmentActivity
import androidx.recyclerview.widget.RecyclerView
import com.meetfriend.app.api.post.model.ShortsPageState
import com.meetfriend.app.responseclasses.video.DataVideo
import io.reactivex.Observable
import io.reactivex.subjects.PublishSubject

class PlayShortsAdapter(
    private val context: Context,
    private val requireActivity: FragmentActivity
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private val playShortsViewClicksSubject: PublishSubject<ShortsPageState> =
        PublishSubject.create()
    val playShortsViewClicks: Observable<ShortsPageState> = playShortsViewClicksSubject.hide()

    private val shortsViewByUserSubject: PublishSubject<Int> =
        PublishSubject.create()
    val shortsViewByUser: Observable<Int> = shortsViewByUserSubject.hide()

    private var adapterItems = listOf<AdapterItem>()

    var listOfDataItems: List<DataVideo>? = null
        set(listOfReelsInfo) {
            field = listOfReelsInfo
            updateAdapterItems()
        }

    @Synchronized
    private fun updateAdapterItems() {
        val adapterItems = mutableListOf<AdapterItem>()

        listOfDataItems?.forEach {
            adapterItems.add(AdapterItem.PlayShortsViewItem(it))
        }

        this.adapterItems = adapterItems
        notifyDataSetChanged()
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ViewType.PlayShortsViewItemType.ordinal -> {
                PlayReelAdapterViewHolder(
                    PlayShortsView(context).apply {
                        playShortsViewClicks.subscribe { playShortsViewClicksSubject.onNext(it) }
                    }
                )
            }

            else -> throw IllegalArgumentException("Unsupported ViewType")
        }
    }

    override fun getItemCount(): Int {
        return adapterItems.size
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        val adapterItem = adapterItems.getOrNull(position) ?: return
        when (adapterItem) {
            is AdapterItem.PlayShortsViewItem -> {
                shortsViewByUserSubject.onNext(adapterItem.dataVideo.posts_id)
                (holder.itemView as PlayShortsView).bind(
                    adapterItem.dataVideo
                )
            }
        }
    }

    private class PlayReelAdapterViewHolder(view: View) : RecyclerView.ViewHolder(view)

    sealed class AdapterItem(val type: Int) {
        data class PlayShortsViewItem(var dataVideo: DataVideo) :
            AdapterItem(ViewType.PlayShortsViewItemType.ordinal)
    }

    private enum class ViewType {
        PlayShortsViewItemType
    }
}
