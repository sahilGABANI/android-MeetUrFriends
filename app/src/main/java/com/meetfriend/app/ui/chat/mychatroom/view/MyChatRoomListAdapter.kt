...
class MyChatRoomListAdapter(private val context: Context) :
    ...
    private fun updateAdapterItems() {
        ...
        listOfDataItems?.forEach {
            adapterItems.add(AdapterItem.MyChatRoomItem(it))
        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ViewType.MyChatRoomItemType.ordinal -> {
                MyChatRoomAdapterViewHolder(
                    MyRoomListView(context).apply {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (adapterItem) {
            is AdapterItem.MyChatRoomItem -> {
                (holder.itemView as MyRoomListView).bind(adapterItem.chatRoomInfo)
            }
        }
    }

    private class MyChatRoomAdapterViewHolder(view: View) : RecyclerView.ViewHolder(view)

    sealed class AdapterItem(val type: Int) {
        data class MyChatRoomItem(var chatRoomInfo: ChatRoomInfo) :
            AdapterItem(ViewType.MyChatRoomItemType.ordinal)
    }
