...
class GiftsHistoryAdapter(private val context: Context) :
    ...
    var listOfDataItems: List<GiftTransactionInfo>? = null
        set(listOfReelsInfo) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ViewType.GiftHistoryItemType.ordinal -> {
                GiftHistoryAdapterViewHolder(GiftsHistoryView(context).apply {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (listItem) {
            is ListItem.GiftHistoryItem -> {
                (holder.itemView as GiftsHistoryView).bind(listItem.giftTransactionInfo)
            }
        }
    }
    ...
    sealed class ListItem(val type: Int) {
        data class GiftHistoryItem(var giftTransactionInfo: GiftTransactionInfo) :
